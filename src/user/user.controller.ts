...
// Alias internal: / (untuk support mount point /api/v1/profile)
 ...
 * @access  Private
 */
const uploadPhotoHandler = async (req: Request, res: Response) => {
  try {
    const userId = req.user?.userId;
...
// Alias internal: /photo (untuk support mount point /api/v1/profile)
router.post("/photo", authenticate, upload.single("photo"), uploadPhotoHandler);

const deletePhotoHandler = async (req: Request, res: Response) => {
  try {
    const userId = req.user?.userId;
 ...
 * @access  Private
 */
router.delete("/profile/photo", authenticate, deletePhotoHandler);

// Alias internal: /photo (untuk support mount point /api/v1/profile)
router.delete("/photo", authenticate, deletePhotoHandler);
